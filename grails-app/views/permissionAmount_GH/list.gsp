...
<div id="list-permissionAmount_GH" class="content scaffold-list" role="main">
    ...
    <g:form action="save">
        ...
        <div class="fieldcontain bank-region-percent fix-at-top">
            ...
            <span class="property-label-my max-growth"><g:message code="permissionAmount_GH.permAmountCurrent" /></span>
            <span class="property-label-my max-growth"><g:message code="permissionAmount_GH.permAmountCurrent" /></span>
            <span class="property-label-my min-growth"><g:message code="used.amount"/></span>
            <span class="property-label-my max-growth"><g:message code="permissionAmount_GH.permAmountCurrent.last" /></span>
        ...
        <g:each in="${branchs}">
            <div class="fieldcontain bank-region-percent">
                <g:set var="permissionAmount"
                       value="${cashmanagement.PermissionAmount_GH.findByBranchAndPermissionDate(it, resultParm.date)}"/>
                <g:set var="usedAmount"
                       value="${cashmanagement.LoanRequest_GH.findAllByBranchAndRequestDateGreaterThanEqualsAndLoanRequestStatusInList(it, resultParm.date, [LoanRequest_GH.Confirm,LoanRequest_GH.Paid]).sum {it.loanAmount} ?: 0}"/>
                <g:set var="permissionAmounts"
                       value="${cashmanagement.PermissionAmount_GH.findAllByBranchAndPermissionDateLessThan(it, resultParm.date).sort {it.permissionDate}}"/>
                <g:set var="permissionAmountLast"
                       value="${permissionAmounts?permissionAmounts.last():null}"/>
                <g:set var="usedAmountLast"
                       value="${cashmanagement.LoanRequest_GH.findAllByBranchAndRequestDateLessThanAndRequestDateGreaterThanEqualsAndLoanRequestStatusInList(it, resultParm.date,(permissionAmountLast?.permissionDate)?:new Date(), [LoanRequest_GH.Confirm,LoanRequest_GH.Paid]).sum {it.loanAmount} ?: 0}"/>
